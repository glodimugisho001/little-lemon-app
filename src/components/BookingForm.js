...
export default function BookingFom({ AvailableTime, setAvalaibleTime, AvailableTimes, dispatch}) {
  ...
  const validateForm = () => {
    ...
    if (!occasion || occasion === "--Select an Occasion--") {
      newErrors.occasion = "Veuillez choisir une occasion";
    }
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // On marque tous les champs comme touchés lors de la soumission
    setTouched({
      date: true,
      time: true,
      guests: true,
  ...
  // Valider le formulaire quand les valeurs changent
  useEffect(() => {
    validateForm();
